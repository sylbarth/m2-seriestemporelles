...
def plot_tree(data, tree_to_plot):
...
class TimeSeriesRandomForestRegressor(TimeSeriesModel):
...
class TimeSeriesRandomForestClassifier(TimeSeriesModel):

    def fit(self, random_state=42):
        self.model = RandomForestClassifier(n_estimators=100, random_state=random_state, oob_score=True)
        self.fit = self.model.fit(self.X_train, self.y_train)
        return self.fit